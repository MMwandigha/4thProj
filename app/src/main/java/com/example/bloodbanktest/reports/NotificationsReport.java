...
import com.example.bloodbanktest.Model.Notification;
...
public class NotificationsReport extends AppCompatActivity {
    ...
    List<Notification> paymentUsersList;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        getSupportActionBar().setTitle("Notifications Reports");

        mAuth = FirebaseAuth.getInstance();
        userRef = FirebaseDatabase.getInstance().getReference().child("notifications");
        payRef = FirebaseDatabase.getInstance().getReference().child("notifications").child(FirebaseAuth.getInstance().getCurrentUser().getUid());
        ...
        pFile = new File(payfile, "Notifications.pdf");
        fetchPaymentUsers();
...
//creating an object and setting to displlay
                    Notification pays = new Notification();
                    pays.setReceiverId(snapshot.child("idnumber").getValue().toString());
                    pays.setSenderId(snapshot.child("name").getValue().toString());
                    pays.setText(snapshot.child("email").getValue().toString());
                    pays.setDate(snapshot.child("phonenumber").getValue().toString());
...
//                    Log.d("Payment")
    ...
    private void createPaymentReport(List<Notification> paymentUsersList) throws DocumentException, FileNotFoundException {
        ...
        PdfPTable table = new PdfPTable(new float[]{6, 20, 20, 20, 20});
        ...
        Chunk idText = new Chunk("Receiver ID", white);
        ...
        for (int i = 0; i < paymentUsersList.size(); i++) {
            Notification pay = paymentUsersList.get(i);

            String id = String.valueOf(i + 1);
            String idnumber = pay.getReceiverId();
            String name = pay.getSenderId();
            String email = pay.getText();
            String phonenumber = pay.getDate();
